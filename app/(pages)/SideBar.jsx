...
import { HiChevronDoubleLeft, HiChevronDoubleRight, HiHeart, HiMusicNote, HiStar } from "react-icons/hi";
    ...
    return (
        <aside
            className={classNames({
                "lg:flex hidden flex-col sticky z-50 left-0 top-0 bottom-0 overflow-y-auto h-screen border border-main": true,
                "w-[16rem] text-[0.8rem]": isExtended == true,
                "w-[5rem]": isExtended === false,
            })}
        >
            {/* Logo Part */}
            <div
                className={classNames({
                    "flex w-full items-center border-b border-main transition-all duration-300": true,
                    "h-28 p-8": isExtended === true,
                    "h-20 p-7": isExtended === false,
                })}
            >
                <Link href="/">
                    {isExtended ? (
            ...
            <div className="flex flex-col flex-1 grow">
                {/* Search Part */}
                <div className="flex w-full items-center border-b border-main p-4">
                    {Menu["Search"].map((nav, index) => {
                        return (
                            <button
                                key={index}
                                className={classNames({
                                    "w-full h-8 rounded-xl px-4 ": true,
                                    "flex items-center gap-3 transition-colors": true,
                                    active: nav.href === currentPathname,
                                })}
                            >
                                {nav.icon}
                                {isExtended && <span>{nav.label}</span>}
                            </button>
                        );
                    })}
                </div>
                ...
                {/* Music Part */}
                <div className="flex flex-col w-full items-center border-b border-main p-4 space-y-1">
                    {Menu["Music"]?.map((nav, index) => {
                        return (
                            <Link
                                key={index}
                                href={nav.href}
                ...
                {/* Additional Part */}
                <div className="flex grow flex-col w-full items-center border-b border-main p-4 space-y-1">
                    {Menu["Additional"].map((nav, index) => {
                        return (
                            <Link
                                key={index}
                                href={nav.href}
            ...
            <div className="flex flex-col items-center">
                <div className="w-full p-4 border-b border-main">
                    <button className="w-full h-8 text-xs bg-[#5765f2] flex items-center gap-3 px-4 rounded-xl">
                        <FaDiscord size={!isExtended ? 30 : 17} />
                        {isExtended && <span>Join our community</span>}
                    </button>
                </div>
            </div>
            ...
            <div className="flex flex-col items-center">
                <div
                    className={classNames({
                        "w-full flex items-center p-4 border-b border-main": true,
                        "flex-col-reverse": !isExtended,
                    })}
                >
                    <button
                        onClick={toggleSideBar}
                        className={classNames({
                            "p-2 rounded-full hover:bg-main transition-transform": true,
                            "rotate-180": !isExtended,
                            "rotate-0": isExtended,
                        })}
                    >
                        <HiChevronDoubleLeft size={26} />
                    </button>
                    <button className="p-2 rounded-full hover:bg-main">
                        <LuMoonStar size={23} />
                    </button>
                </div>
            </div>
